...
window.onload = function init()
{
	...
	document.getElementById("slider").onchange = function(event) {
		...
		points = []; // 清空之前的点
		colorsOfVertexs = []; // 清空颜色数据
		divideTriangle(vertices[0], vertices[1], vertices[2], numTimesToSubdivide); // 重新生成顶点
		sliderchangeflag = true; // 设置标志，告诉告诉 `render()` 需要更新		
	...
	canvas.addEventListener("mousedown", function(event){
		...
		//--------------------------------------------------------------------------------
        // 将鼠标位置转换为WebGL的canvas坐标系，范围 [-1, 1]
        var canvasX = (x - rect.left) / canvas.width * 2 - 1;
        var canvasY = -(y - rect.top) / canvas.height * 2 + 1;
	    centerX = canvasX;
	    centerY = canvasY;
    ...
    frame++;
     ...
     if (now > 1000 + lastTime) {
        ...
        document.getElementById("label").innerHTML = 
            "FPS: " + fps + " | 顶点数: " + points.length;  // 显示顶点数
        frame = 0;
        lastTime = now;
    };
