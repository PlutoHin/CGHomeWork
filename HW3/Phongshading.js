...
var fogEnabled = true;
var fogMode = 2; // 0 linear, 1 exp, 2 exp2
var fogColor = vec3(0.95, 0.82, 0.90); // 浅蓝色雾
var fogDensity = 0.03; // exp/exp2 的密度
var fogNear = 5.0; // linear 起始
var fogFar = 40.0; // linear 终止
...
window.onload = function() {
	// 获取 canvas 并初始化 WebGL2
	canvas = document.getElementById("canvas");
	gl = canvas.getContext('webgl2');
	if (!gl) { alert("WebGL2 isn't available"); return; }

	// 设置 canvas 尺寸和 viewport
...
function render(){
	...
	// fog uniforms
	gl.uniform1i( gl.getUniformLocation(program, "u_fogEnabled"), fogEnabled ? 1 : 0 );
	gl.uniform1i( gl.getUniformLocation(program, "u_fogMode"), fogMode );
	gl.uniform3fv( gl.getUniformLocation(program, "u_fogColor"), flatten(fogColor) );
...
window.onkeydown = function(e){
	...
	switch (code) {
		...
		case 70: // F 键：切换雾开关
			...
			break;
		case 71: // G 键：切换雾模式（linear -> exp -> exp2）
			fogMode = (fogMode + 1) % 3;
			break;case 70: // F 键：切换雾开关
			fogEnabled = !fogEnabled;
			break;
		case 71: // G 键：切换雾模式（linear -> exp -> exp2）
			fogMode = (fogMode + 1) % 3;
